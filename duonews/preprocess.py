...
from . import DUONEWS_DIR, DAILY_BRIEF
...
def generate_brief(date_str=None, top_n=30):
    ...
    # ── Load historical judgment baseline (P0-3: orphan function wiring) ──
    judgment_baseline: dict = {}
    try:
        from . import find_recent_report, extract_judgment_baseline
        prior_report = find_recent_report(today, max_lookback=7)
        if prior_report:
            judgment_baseline = extract_judgment_baseline(prior_report)
            print(f"[preprocess] Judgment baseline loaded from {judgment_baseline.get('date', '?')}",
                  file=sys.stderr)
    except Exception as e:
        print(f"[preprocess] Judgment baseline skipped: {e}", file=sys.stderr)

    # ── Load cross-day discoveries for injection into brief ──────────────
    cross_day_discoveries: list[dict] = []
    try:
        cross_day_discoveries = db.get_cross_day_discoveries(run_date=today, limit=20)
    except Exception:
        pass

    # Load ranked snippets (scored by snippet_scorer)
    try:
        from harness.snippet_scorer import score_snippets
    ...
    # ── Cross-day discoveries section (for report_writer consumption) ──
    if cross_day_discoveries:
        lines.extend([
            "---",
            "",
            "## 🔍 语义嗅探原始数据",
        ...
        lines.append("| # | 共享实体 | 历史日期 | 重叠率 | 稀有度 | 类型 |")
        lines.append("|---|----------|----------|--------|--------|------|")
        for i, d in enumerate(cross_day_discoveries[:10]):
            entities = ", ".join(d.get("shared_entities", [])[:3])
            hist_date = d.get("history_date", "?")
            cross_tag = "跨板块" if d.get("cross_section", 1) > 1 else "同板块"
            lines.append(
                f"| {i+1} | {entities} | {hist_date} | "
                f"{d.get('jaccard', 0):.2f} | {d.get('rarity', 0):.3f} | {cross_tag} |"
            )
        lines.append("")

    # ── Historical judgment baseline (for report_writer consumption) ──
    if judgment_baseline and judgment_baseline.get("headline_judgment"):
        lines.extend([
            "---",
            "",
            "## 📜 历史判断基线",
        ...
        lines.append(f"**来源日期**: {judgment_baseline.get('date', '未知')}")
        lines.append(f"**头条判断**: {judgment_baseline.get('headline_judgment', '')}")
        lines.append("")
        if judgment_baseline.get("key_entities"):
            lines.append(f"**关注实体**: {', '.join(judgment_baseline['key_entities'][:10])}")
            lines.append("")
        if judgment_baseline.get("hypotheses"):
            lines.append("**历史假设**:")
            for h in judgment_baseline["hypotheses"][:5]:
                lines.append(f"- {h}")
            lines.append("")
        if judgment_baseline.get("prophet_signals"):
            lines.append("**活跃 Prophet 信号**:")
            lines.append("| ID | 预言 | 时间窗口 | 置信度 | 验证标准 | 状态 |")
            lines.append("|----|------|----------|--------|----------|------|")
            for s in judgment_baseline["prophet_signals"]:
                lines.append(
                    f"| {s.get('id', '?')} | {s.get('claim', '')[:50]} | "
                    f"{s.get('time_horizon_days', '?')}天 | {s.get('confidence', '?')} | "
                    f"{s.get('verification_criteria', '')[:30]} | {s.get('status', '?')} |"
                )
            lines.append("")
    ...
    lines.extend([
        ...
        "## 📋 待完成",
        "",
        "> `report_writer` 将自动生成完整日报。剩余手动步骤：",
        "",
        "1. **今日判断** — 审阅自动生成内容，必要时修改 ≤100字",
        "2. **重点分析** — 补充深度因果分析（report_writer 自动组装数据）",
        "3. **推送确认** — 确认飞书推送内容",
        "",
    ])
